...
///
/// compute_rain_collected computes the amount of water that can be collected in a terrain.
/// The terrain is represented as a vector of elevations. The elevation at each index of the vector
/// represents the height of the terrain at that point. Negative elevations are allowed.
/// 
/// # Arguments
/// 
/// * `terrain` - A slice of i64 representing the terrain.
/// 
/// # Returns
/// 
/// The amount of water that can be collected in the terrain.
/// 
/// # Example
/// 
/// ```
/// use rain_collected::trap;
/// 
/// let terrain = [1, 4, 2, 5, 3, 6, 4, 7];
/// let water_collected = compute_rain_collected(&terrain);
/// assert_eq!(water_collected, 6);
/// ``` 
pub fn compute_rain_collected(terrain: &[i64]) -> u64 {
    let n = terrain.len();
    if n < 3 {
        return 0;
    }
...
pub fn compute_rain_collected_parallel(terrain: &[i64]) -> u64 {
    let n = terrain.len();
    if n < 3 {
        return 0;
    }
